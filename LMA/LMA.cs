...
namespace Net.Kniaz.LMA
{
    ...
    public class LMA
    {
        ...
        private double minDeltaChi2 = 1e-7;
        ...
        public LMA LMA_Create(LMAFunction function, double[] parameters,
                    double[] PayerSwaption, double[] weights, int numofvols,
                    double argDeltaChi2, int argMaxIter)
        {
            Double[] Idx = new Double[PayerSwaption.Length];
            for (int i = 0; i < PayerSwaption.Length; i++)
            {
                Idx[i] = i;
            }
            Double[][] dataPoints = new double[][] { Idx, PayerSwaption };
            GeneralMatrix alpha = new GeneralMatrix(numofvols, numofvols);
            LMA result = new LMA(function, parameters, dataPoints, weights, alpha, argDeltaChi2, argMaxIter);
            // result.Fit();
            ExecuteThread(result);
            return result;
        }
        #endregion
        ...
        public void ExecuteThread(Object pParam)
        {
            CreateThread(pParam);
        }

        public void CreateThread(Object pParam)
        {
        ...
        private void ThreadRun(Object pParam)   // Thread 함수 실행
        {
            //String lstParam = (String)pParam;
            LMA lstParam = (LMA)pParam;
            lstParam.Fit();
            //Thread.Sleep(10000);
        }

        public bool return_isfitted()
        {
            return isFitted;
        }
